...
public class RYSOreFeature extends Feature<RYSOreFeatureConfig> {
    public RYSOreFeature(Codec<RYSOreFeatureConfig> codec) {
        super(codec);
    }

    @Override
    public boolean generate(ServerWorldAccess serverWorldAccess, StructureAccessor structureAccessor,
            ChunkGenerator chunkGenerator, Random random, BlockPos blockPos, RYSOreFeatureConfig oreFeatureConfig) {
        ...
        for (int s = n; s <= n + q; ++s) {
            for (int t = p; t <= p + q; ++t) {
                if (o <= serverWorldAccess.getTopY(Heightmap.Type.OCEAN_FLOOR_WG, s, t)) {
                    return this.generateVeinPart(serverWorldAccess, random, oreFeatureConfig, d, e, h, j, l, m, n, o, p,
                            q, r);
                }
            }
        }
    ...
    protected boolean generateVeinPart(WorldAccess world, Random random, RYSOreFeatureConfig config, double startX,
