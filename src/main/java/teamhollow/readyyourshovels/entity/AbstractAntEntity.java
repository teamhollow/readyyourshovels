...
import teamhollow.readyyourshovels.registry.RYSPointOfInterests;
...
import teamhollow.readyyourshovels.tag.RYSItemTags;
...
public abstract class AbstractAntEntity extends AnimalEntity {
    ...
    private boolean isNestValid() {
        if (!this.hasNest()) {
            ...
            TileEntity blockEntity = this.world.getTileEntity(this.nestPos);
            return blockEntity != null && blockEntity.getType() == RYSTileEntity.ANT_NEST;
        }
    }
    ...
    private void setAntFlag(int bit, boolean value) {
        if (value) {
            this.dataManager.set(multipleByteTracker, (byte) ((Byte) this.dataManager.get(multipleByteTracker) | bit));
        } else {
            this.dataManager.set(multipleByteTracker, (byte) ((Byte) this.dataManager.get(multipleByteTracker) & ~bit));
        }
    ...
    private boolean getAntFlag(int location) {
        return ((Byte) this.dataManager.get(multipleByteTracker) & location) != 0;
    }
    ...
    class EnterNestGoal extends Goal {
        ...
        public boolean shouldExecute() {
            if (AbstractAntEntity.this.hasNest() && AbstractAntEntity.this.canEnterNest() && AbstractAntEntity.this.nestPos.withinDistance(AbstractAntEntity.this.getPositionVec(), 30.0D)) {
                ...
                if (blockEntity instanceof AntNestTileEntity) {
                    AntNestTileEntity AntNestTileEntity = (teamhollow.readyyourshovels.tileentity.AntNestTileEntity) blockEntity;
    ...
    public class MoveToAntNestGoal extends Goal {
        ...
        private boolean startMovingToFar(BlockPos pos) {
            ...
            AbstractAntEntity.this.navigator.tryMoveToXYZ((double) pos.getX(), (double) pos.getY(), (double) pos.getZ(), 1.0D);
            ...
            if (this.ticks > 600) {
                ...
                if (!isWithinRangeOfTarget) {
                    ...
                    if (this.ticks > 600)
                        ...
                        if (isWithinRangeOfTarget) {
                            AbstractAntEntity.this.getLookController().setLookPosition(vec3d.getX(), vec3d.getY(), vec3d.getZ());
                        }
                ...
                for (int j = 0; (double) j < searchDistance; ++j) {
                    for (int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                        for (int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                            mutable.setAndOffset((Vector3i) blockPos, k, i - 1, l);
        ...
        public void startExecuting() {
            ...
            if (vec3d != null) {
                AbstractAntEntity.this.navigator.setPath(
                        AbstractAntEntity.this.navigator.getPathToPos((BlockPos) (new BlockPos(vec3d)), 1), 1.0D);
            }
